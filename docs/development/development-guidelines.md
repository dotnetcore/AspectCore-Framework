...
- 理解 nullable reference types 的契约意义（本项目 `src/` 中仅 `SourceGenerator` 启用 nullable，测试项目中仅 `AspectCore.E2E.Tests` 启用）
...
- 部分测试项目（E2E.Tests）和 SourceGenerator 已启用 nullable，`string` 和 `string?` 是契约差异
- 不要把 `!` 当"修编译"的万能胶
