...
- **SourceGenerator（编译期）**：显式 opt-in，编译时由 Roslyn 生成代理类型，降低代理生成期的动态代码依赖（适合裁剪场景；但拦截/构造仍有运行时反射约束，非端到端 NativeAOT）。见 [Source Generator 编译时引擎](../architecture/source-generator.md)。
