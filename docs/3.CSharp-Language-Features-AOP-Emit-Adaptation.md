...
P0（仍待处理）             P1（功能缺失）            P2（注解/低优先）
┌─────────────────┐      ┌─────────────────┐      ┌─────────────────┐
│ init accessor   │      │ ref 字段/scoped  │      │ ✅ NRT 注解完善   │
│ required 成员    │      │ 部分属性         │      │ ✅ 泛型属性转发   │
│ record 类型      │      │ 插值处理器       │      │ ✅ CallerArgument │
│ ref return       │      │ Index/Range 测试 │      │ ✅ 集合表达式       │
│                  │      │                 │      │ ✅ Experimental     │
│                  │      │                 │      │ ✅ 类型参数属性     │
│                  │      │                 │      │ ✅ 方法组自然类型   │
└─────────────────┘      └─────────────────┘      └─────────────────┘

✅ 已完成（P0）：异步流（IAsyncEnumerable<T>）和 IAsyncDisposable
✅ 已完成（P1）：ref struct/scoped、主构造函数、params 集合
...
| 类别 | 数量 | 关键项 |
|------|------|--------|
| ✅ 已适配 | 20 类 | 协变返回、async/await、IAsyncEnumerable、IAsyncDisposable、泛型、Nullable\<T\>、ref/out/in 参数、ValueTuple、NRT、泛型属性、CallerArgumentExpression、集合表达式、Experimental、类型参数属性、方法组自然类型、主构造函数、params 集合、ref struct/scoped |
| 🔴 P0 需适配 | 4 项 | init、required、record、ref return |
| 🟡 P1 需适配 | 3 项 | 部分属性、插值处理器、Index/Range 测试 |
| 🟢 P2 已适配 | 7 项 | ✅ NRT、泛型属性、CallerArgumentExpression、集合表达式、Experimental、类型参数属性、方法组自然类型 |
| ⚪ 无需适配 | 30+ 项 | 纯语法糖，编译为标准 IL |

**当前高优先级风险点**：`init` + `required`（C# 11+ 推荐模式）、`record` 类型和 `ref` 返回；这些特性仍应作为后续适配重点。
