...
| C# 13.0 | ✅ params 集合（params IEnumerable\<T\>） | DynamicProxy：`[ParamCollection]` 属性随自定义属性自动转发（不在跳过列表中）；SG：`EmitParameterDecl` 检测 `IsParams` 并 emit `params` 关键字；`ParamCollectionAttribute` 加入 `ForwardedAttributeNames` 白名单 | `ProxyEmitter.cs:882-893,1054-1059` |
...
| Source Generator | `EmitParameterDecl` 检测 `p.IsParams` 并 emit `params` 关键字（覆盖 `params IEnumerable<T>`、`params T[]`、`params ReadOnlySpan<T>`） | `ProxyEmitter.cs:882-893` |
| Source Generator | `ParamCollectionAttribute` 加入 `ForwardedAttributeNames` 白名单，确保属性被转发 | `ProxyEmitter.cs:1054-1059` |
...
┌─────────────────┐      ┌─────────────────┐      ┌─────────────────┐
│ IAsyncEnumerable │      │ ref 字段/scoped  │      │ ✅ NRT 注解完善   │
│ init accessor    │      │ 部分属性         │      │ ✅ 泛型属性转发   │
│ required 成员    │      │ 插值处理器       │      │ ✅ CallerArgument  │
│ record 类型      │      │ Index/Range 测试 │      │ ✅ 集合表达式       │
│ ref struct       │      │                 │      │ ✅ Experimental     │
│ ref return       │      │                 │      │ ✅ 类型参数属性     │
│                  │      │                 │      │ ✅ 方法组自然类型   │
└─────────────────┘      └─────────────────┘      └─────────────────┘

✅ 已完成（P1）：主构造函数、params 集合
...
| 类别 | 数量 | 关键项 |
|------|------|--------|
| ✅ 已适配 | 17 类 | 协变返回、async/await、泛型、Nullable\<T\>、ref/out/in 参数、ValueTuple、NRT、泛型属性、CallerArgumentExpression、集合表达式、Experimental、类型参数属性、方法组自然类型、主构造函数、params 集合 |
| 🔴 P0 需适配 | 6 项 | IAsyncEnumerable、init、required、record、ref struct、ref return |
| 🟡 P1 需适配 | 4 项 | ref 字段/scoped、部分属性、插值处理器、Index/Range |
...
> **更新记录**：P2 全部 7 项已在 commit `2733202` 中适配完成；P1 主构造函数和 params 集合已在本次适配完成。
