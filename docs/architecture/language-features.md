...
> **注意**：`params ReadOnlySpan<T>` 涉及 ref struct 参数装箱问题（P0 3.1.5），当前不支持通过 `object[]` 传递 ref struct 参数，需待 ref struct 适配后支持。
...
✅ 已完成（P1）：ref struct/scoped、主构造函数、params 集合、部分属性、插值处理器、Index/Range
...
| 类别 | 数量 | 关键项 |
|------|------|--------|
| ✅ 已适配 | 27 类 | 协变返回、async/await、IAsyncEnumerable、IAsyncDisposable、init-only、required、部分属性、插值处理器、Index/Range、泛型、Nullable\<T\>、ref/out/in 参数、ValueTuple、NRT、泛型属性、CallerArgumentExpression、集合表达式、Experimental、类型参数属性、方法组自然类型、主构造函数、params 集合、ref struct/scoped、record 类型、ref return |
| 🔴 P0 需适配 | 0 项 | — |
| 🟡 P1 需适配 | 0 项 | — |
