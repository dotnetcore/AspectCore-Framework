...
ref struct（如 `Span<T>`、`ReadOnlySpan<T>`）有 CLR 强制限制：不能装箱、不能作为接口实现、不能作为类字段、不能用于 `async`/`iterator` 方法。若不处理，DynamicProxy 会在 IL 中对 ref struct 触发装箱，导致 `TypeLoadException`。

**实现结果**

- DynamicProxy：`ProxyTypeGenerator` 在入口检查 `type.IsByRefLike` 并拒绝把 ref struct 作为代理目标类型（`RejectRefStruct`），提前给出明确错误而非运行时崩溃。
- Source Generator：`AspectCoreProxyGenerator` 对 `IsRefLikeType` 的候选类型报告 ACSG008 并跳过生成。
- 方法参数/返回值中的 ref struct 由既有的 `EmitLoadArg` 直接传递路径处理，不做 `object` 装箱。

**已知边界**：`params ReadOnlySpan<T>` 等 byref-like 的 `params` 参数在 SG 侧报告 ACSG009 并跳过生成（见 3.2.2）。
